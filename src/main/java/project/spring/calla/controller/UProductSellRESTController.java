...
@RestController
@RequestMapping(value = "/uProduct/sell")
public class UProductSellRESTController {
	private static final Logger logger = LoggerFactory.getLogger(UProductSellRESTController.class);
	...
	public ResponseEntity<Integer> deleteuProductsell(@PathVariable("uProductBuyId") int uProductBuyId) {
		...
		int result = 0;
		try {
			result = uproductService.deleteUProductsell(uProductBuyId);
		} catch (Exception e) {
